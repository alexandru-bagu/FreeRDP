...
{
	WLog_VRB(TAG, "get_piped_char begin (console: %d)", from_console);
	fflush(stdout);

	if (from_console)
	{
		int chr = _getch();
		WLog_VRB(TAG, "_getch = %d", chr);
		fflush(stdout);
		return chr;
	}
	else
...
char* freerdp_passphrase_read(const char* prompt, char* buf, size_t bufsiz, int from_stdin)
{
	...
	WLog_VRB(TAG, "stdin isTty %d", isTty);
	fflush(stdout);
	if (from_stdin)
	{
		printf("%s ", prompt);
		...
		while (read_cnt < bufsiz - 1 && (chr = get_piped_char(isTty)) && chr != NEWLINE &&
		       chr != CARRIAGERETURN)
		{
			WLog_VRB(TAG, "get_piped_char current size: %d, chr: %d", read_cnt, chr);
			fflush(stdout);
			if (chr == BACKSPACE)
			{
				if (read_cnt > 0)
